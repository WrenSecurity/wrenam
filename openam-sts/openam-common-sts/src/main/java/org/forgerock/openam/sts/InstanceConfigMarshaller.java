...
/**
 * Interface to represent the concerns of marshalling to {@code RestSTSInstanceConfig} and 
 * {@code SoapSTSInstanceConfig} instances to/from either the {@code Map<String, Set<String>>}
 * SMS representation or the json representation. Simply wraps the static methods in the
 * {@code RestSTSInstanceConfig} and {@code SoapSTSInstanceConfig} classes, so that they can be easily mocked.
 */
public interface InstanceConfigMarshaller<T extends STSInstanceConfig> {
    /**
     * Called to marshal an {@code STSInstanceConfig} subclass instance to the {@code Map<String, Set<String>>}
     * required for SMS persistence.
     ...
     * to publish the Rest or Soap STS instance. The invocation payload is the SMS {@code Map<String, Set<String>>}
     * representation native to the ViewBean context, wrapped in a JsonValue, so that it may be posted to CREST.
     * This method defines the contract to marshal this representation back to a STSInstanceConfig subclass.
