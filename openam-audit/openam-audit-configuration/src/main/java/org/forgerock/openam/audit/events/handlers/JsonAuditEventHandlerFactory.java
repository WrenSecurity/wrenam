...
public final class JsonAuditEventHandlerFactory implements AuditEventHandlerFactory {
    ...
    public AuditEventHandler create(final AuditEventHandlerConfiguration configuration) throws AuditException {
        ...
        jsonHandlerConfiguration.setEnabled(
            getBooleanMapAttr(attributes, "enabled", true));
    ...
    private void setFileRotationPolicies(JsonAuditEventHandlerConfiguration jsonHandlerConfiguration,
        ...
        jsonHandlerConfiguration.getFileRotation().setRotationEnabled(
            getBooleanMapAttr(attributes, "rotationEnabled", true));
        jsonHandlerConfiguration.getFileRotation().setMaxFileSize(
            getLongMapAttr(attributes, "rotationMaxFileSize", 100000000L, DEBUG));
        jsonHandlerConfiguration.getFileRotation().setRotationFilePrefix(
            getMapAttr(attributes, "rotationFilePrefix", ""));
        jsonHandlerConfiguration.getFileRotation().setRotationFileSuffix(
            getMapAttr(attributes, "rotationFileSuffix", "-yyyy.MM.dd-HH.mm.ss"));
        jsonHandlerConfiguration.getFileRotation().setRotationInterval(
            parseRotationInterval(getMapAttr(attributes, "rotationInterval", "-1")));
    ...
    private void setFileRetentionPolicies(JsonAuditEventHandlerConfiguration jsonHandlerConfiguration,
        ...
        jsonHandlerConfiguration.getFileRetention().setMaxNumberOfHistoryFiles(
            getIntMapAttr(attributes, "retentionMaxNumberOfHistoryFiles", 1, DEBUG));
        jsonHandlerConfiguration.getFileRetention().setMaxDiskSpaceToUse(
            getLongMapAttr(attributes, "retentionMaxDiskSpaceToUse", -1L, DEBUG));
        jsonHandlerConfiguration.getFileRetention().setMinFreeSpaceRequired(
            getLongMapAttr(attributes, "retentionMinFreeSpaceRequired", -1L, DEBUG));
    }
    ...
    private EventBufferingConfiguration getBufferingConfiguration(Map<String, Set<String>> attributes) {
        ...
        bufferingConfiguration.setMaxSize(
            getIntMapAttr(attributes, "maxSize", 10000, DEBUG));
        bufferingConfiguration.setWriteInterval(
            getIntMapAttr(attributes, "writeInterval", 1000, DEBUG) + " millis");
