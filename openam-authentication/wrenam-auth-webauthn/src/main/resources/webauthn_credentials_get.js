(function () {
    ...
    const toFailure = function (error) {
        ...
        switch (error.name) {
        case "AbortError":
            return { reason: REASON_USER_CANCELLED, message: "WebAuthn request was cancelled." };
        case "NotAllowedError":
            return { reason: REASON_NOT_ALLOWED, message: "WebAuthn request was not allowed or timed out." };
        case "NotFoundError":
        ...
        case "InvalidStateError":
            return { reason: REASON_CREDENTIAL_NOT_FOUND, message: "The selected credential could not be used." };
        case "TypeError":
            return { reason: REASON_VERIFICATION_FAILED, message: "WebAuthn request options are invalid." };
        default:
    ...
    errorButton.addEventListener("click", function () {
        ...
        output.value = JSON.stringify({
            ...
            message: "User cancelled passkey sign-in."
        });
    });
    ...
    try {
        ...
        completeWithError(REASON_VERIFICATION_FAILED, "Failed to parse WebAuthn request options.");
        return;
    }
